...
if ($env:GH_CI_LATEST -eq "true") {
    docker build --isolation hyperv --no-cache --pull -t eisai/eisai/comfy-ui:ltsc2025 -t eisai/comfy-ui:$env:GH_CI_TAG-ltsc2025 .\build
} else {
    docker build --isolation hyperv --no-cache --pull -t eisai/eisai/comfy-ui:$env:GH_CI_TAG-ltsc2025 .\build
}
